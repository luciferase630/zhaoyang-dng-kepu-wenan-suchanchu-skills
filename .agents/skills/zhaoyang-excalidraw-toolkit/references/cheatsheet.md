...
- **Windows JSON arguments**: Prefer `--file element.json` or `--file updates.json` over `--data '<json>'`; PowerShell and CMD quote JSON differently.
- **Destructive operations**: `clear-canvas.cjs` requires `--force` or an interactive `yes` confirmation.
